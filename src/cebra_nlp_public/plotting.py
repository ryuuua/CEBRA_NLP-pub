...
def prepare_plot_labels(
    ...
    if cfg.cebra.conditional == "discrete":
        ...
        if vis_cfg is not None:
            if getattr(vis_cfg, "emotion_colors", None):
                palette = dict(vis_cfg.emotion_colors)
            if getattr(vis_cfg, "emotion_order", None):
                order = list(vis_cfg.emotion_order)
        if order is None:
...
def plot_embedding_distributions(
    embeddings: np.ndarray, labels: np.ndarray, cfg: AppConfig, output_dir: Path
) -> None:
    """Plot per-dimension distributions, color-coded by label."""
    ...
    text_labels = [label_map[label_id] for label_id in labels]
