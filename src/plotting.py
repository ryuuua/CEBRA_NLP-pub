...
def plot_embedding_distributions(embeddings: np.ndarray, labels: np.ndarray, cfg: DictConfig, output_dir: Path):
    """
    ...
    for i in range(dim):
        ...
        sns.kdeplot(
            data=plot_df, x='value', hue='label',
            palette=emotion_colors, hue_order=emotion_order,
            ax=ax, fill=True, common_norm=False, alpha=0.5
        )
    ...
    if axes[0].get_legend() is not None:
        ...
        fig.legend(handles, plot_labels, title='Emotion', bbox_to_anchor=(1.0, 0.9), loc='upper left')
